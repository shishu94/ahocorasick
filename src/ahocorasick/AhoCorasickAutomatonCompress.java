 ...
 * @param <T>
 *          accumulator for matching states NOT WORKING YET (trying to figure out why the compression produce wrong results)
 */
public class AhoCorasickAutomatonCompress<T> {
  ...
  private final int[] charTable = new int[maxChars + 1];
  int charTableSize = 1;//0 is reserved

  private final Accumulator<T> acc;

  public AhoCorasickAutomatonCompress(List<String> wordsToFind, Accumulator<T> acc) {
  ...
  private int findNextState(int currentState, char ch) {
    int chi = charTable[ch];
    //    if (chi == NOT_FOUND) {
    //      System.out.println("Not found");
    //    }
    // If goto is not defined, use failure function
    if (g[currentState][chi] == -1) {
      int answer = f[currentState];
      while (g[answer][chi] == -1)
        answer = f[answer];
      g[currentState][chi] = g[answer][chi];
    }

    return g[currentState][chi];// in parallel this may be concurrent, but since any
  ...
  private int convertChar(char ch) {
    if (charTable[ch] == -1) {
      charTable[ch] = charTableSize++;
    }
    return charTable[ch];
  }

  private void setNotPresentChars() {
    for (int i = 0; i < charTable.length; i++) {
      if (charTable[i] == -1) {
        charTable[i] = NOT_FOUND;
      }
    }
  }
  ...
  private void buildAutomaton() {
...

    {
      ...
      for (int w = 0; w < words.size(); w++) {
        ...
        for (int i = 0; i < word.length(); i++) {
          int ch = convertChar(word.charAt(i));
    ...
    // 0 itself
    for (int ch = 0; ch <= charTableSize; ++ch)
      if (g[0][ch] == -1)
        g[0][ch] = 0;
